...
- 已完成基础工程、数据库、单用户 setup/auth/session、首次设置向导、RSS 入库、OPML API、Web OPML UI、feed folder 展示筛选、feed 管理 UI、后台刷新 job、文章保留清理、设置页、OpenAI-compatible / Ollama embedding provider、embedding job pipeline、sqlite-vec 生产写入/重建、文章列表分页、基础 Web 阅读器、文章动作、Reader Command 清账、baseline ranking、推荐解释和 i18n foundation。
...
| C-27 | Done | P1 | frontend/backend/test/docs | Reader Command bulk read cleanup | `POST /api/reader/commands/mark-scope-read` 可按 latest/recommended/search 当前范围批量标记未读为已读；写入 `reader_command_events`，不写入 `behavior_events`，不进入兴趣画像学习；Web 合并未读 toggle、计数和清账确认控件。 |
...
| R-21 | Done | P1 | frontend/backend/test/docs | Unread debt cleanup | latest/recommended/search 支持当前 scope 清账；清账是 Reader Command，不是 Behavior Event，不作为推荐正反馈；只支持批量已读，不支持批量删除/收藏/不感兴趣。 |
