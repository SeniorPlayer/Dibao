...
- 已完成基础工程、数据库、RSS 入库、OPML API、基础 Web 阅读器、文章动作、baseline ranking、推荐解释和 i18n foundation。
- 尚未完成让普通自托管用户可用的关键闭环：单用户 setup/auth、Web OPML UI、feed 管理 UI、后台刷新、Docker 打包、设置页、embedding provider、embedding job pipeline、sqlite-vec 生产集成、Profile Algorithm v0、推荐排序 v1、E2E 测试。
...
- 移动端 PWA、桌面打包、原生壳、导入迁移、部署升级、备份恢复等长期可用性工作还未开始。
...
| ID | Status | Priority | Type | Task | Acceptance Criteria |
| --- | --- | --- | --- | --- | --- |
| R-01 | Todo | P0 | frontend/backend/test | Web OPML import/export UI | Web 端可选择 `.opml` / `.xml` 文件导入；导入成功后刷新 feed list；展示 created/skipped/errors；可导出 `dibao-subscriptions.opml`；文案接入 i18n；测试覆盖 API client 和关键 UI。 |
| R-02 | Todo | P0 | backend/frontend/test | Single-user setup/auth/session | 首次启动可设置访问密码；登录后使用 httpOnly session cookie；未登录不能访问受保护 API；测试覆盖 setup/login/logout/session。 |
| R-03 | Todo | P0 | frontend/design/test | First-run setup wizard | 首次访问进入设置向导；完成密码设置、OPML 导入或手动添加 feed、provider 选择或跳过；完成后进入阅读器；移动端可用。 |
| R-04 | Todo | P0 | backend/frontend/test | Feed management minimal UI | 用户可查看 feeds/folders、添加 feed、编辑名称、移动分组、启用/禁用、软删除、查看 lastError；操作后列表一致刷新。 |
| R-05 | Todo | P0 | backend/infra/test | Refresh all and background refresh | 提供手动刷新全部 API；支持定时刷新启用 feeds；记录抓取成功/失败状态；失败不会阻塞其他 feed；测试使用 fixture fetcher，不依赖外网。 |
| R-06 | Todo | P0 | backend/infra/test | Job runner foundation | 后台任务具备可启动、可停止、可串行执行、可记录错误的最小框架；可承载 feed refresh 和 embedding jobs；测试覆盖任务失败和重试边界。 |
| R-07 | Todo | P0 | backend/test | Article retention policy | 可配置保留天数，默认可设为 60 天；清理过期文章时保留必要行为/画像统计；软删和硬删规则写入文档并有测试。 |
| R-08 | Todo | P0 | backend/frontend/algorithm/test | Embedding provider configuration | 支持配置 Ollama、OpenAI-compatible endpoint、自定义 HTTP provider；可测试连接、读取模型名/维度、保存设置、展示最近错误。 |
| R-09 | Todo | P0 | algorithm/backend/infra/test | Embedding job pipeline | 新文章进入待 embedding 队列；批量调用 provider；失败可重试；结果写入 authority table；不阻塞 RSS refresh 和阅读。 |
| R-10 | Todo | P0 | algorithm/backend/test | sqlite-vec production adapter | 文章向量可写入、查询、重建；维度不匹配会返回明确错误；支持从 BLOB authority table 重建 vec0 index；测试覆盖 KNN 查询。 |
| R-11 | Todo | P0 | algorithm/backend/test | Profile Algorithm v0 implementation | 行为事件按参数表更新用户画像/兴趣簇；支持增量更新；文章删除不破坏画像统计；测试覆盖正向、负向、衰减和边界行为。 |
| R-12 | Todo | P0 | algorithm/backend/test | Recommendation ranking v1 | 推荐分融合 baseline、embedding similarity、source weight、state、freshness、penalty；无 embedding 时 fallback 到 baseline；排序稳定且可解释。 |
| R-13 | Todo | P0 | frontend/backend/test | Settings page v0 | 用户可配置语言、阅读字号、行高、段距、阅读宽度、数据保留天数、embedding provider；设置持久化并即时影响 UI。 |
| R-14 | Todo | P0 | infra/docs/test | Docker packaging | 提供 Dockerfile / compose 示例；数据目录可挂载；健康检查可用；README 描述部署、升级、备份、恢复；本地构建验证通过。 |
| R-15 | Todo | P0 | test | MVP E2E smoke suite | 覆盖首次设置、OPML 导入、手动刷新、打开文章、收藏/稍后读/不感兴趣、推荐解释、provider 跳过或测试连接；桌面和移动视口至少各一条主流程。 |
| R-16 | Todo | P1 | frontend/backend/test | Search UI with FTS | 用户可搜索文章标题/摘要/正文；支持 feed/folder/state 筛选；空状态和错误状态可用；搜索不影响推荐流状态。 |
| R-17 | Todo | P1 | frontend/design/test | Mobile web layout hardening | 手机浏览器中订阅源、列表、阅读器、设置页可完成主流程；按钮和状态文案不溢出；关键视口截图验证。 |
| R-18 | Todo | P1 | frontend/infra/test | PWA foundation | 提供 manifest、图标、基础 service worker 或离线壳；可添加到主屏；不承诺离线全文阅读。 |
...
| R-20 | Todo | P1 | docs | User documentation v0 | README 覆盖安装、首次设置、OPML、provider、备份、升级、常见问题；非开发者可按步骤完成部署。 |
...
| M1-09 | Todo | P0 | frontend/backend/test | OPML Web UI | 用户可在 Web 中导入/导出 OPML，不需要 API 工具。 |
| M1-10 | Todo | P1 | frontend/backend/test | Feed folders visible in reader | 已导入分组可在 Web 中显示；可按 folder 筛选 articles。 |
| M1-11 | Todo | P1 | frontend/test | Article list pagination UI | latest/recommended 列表可加载更多；cursor 状态稳定；空状态和错误状态清晰。 |
...
| ID | Status | Priority | Type | Task | Acceptance Criteria |
| --- | --- | --- | --- | --- | --- |
| M2-01 | Todo | P0 | backend/frontend/test | Single-user auth | 首次设置密码；登录/退出/session 可用；受保护 API 未登录返回 `UNAUTHORIZED`。 |
| M2-02 | Todo | P0 | frontend/design/test | First-run setup wizard | 用户从空库启动能完成密码、订阅源、provider 跳过或配置，并进入主界面。 |
| M2-03 | Todo | P0 | frontend/backend/test | Feed management UI | feed/folder 的新增、编辑、禁用、删除、移动分组和错误查看可用。 |
| M2-04 | Todo | P0 | backend/infra/test | Background refresh | 启用 feeds 可定时刷新；手动刷新全部可用；失败隔离并记录错误。 |
| M2-05 | Todo | P0 | backend/test/docs | Data retention | 默认保留策略可配置；清理文章时行为数据和画像统计处理规则明确。 |
| M2-06 | Todo | P0 | frontend/design/test | Settings page v0 | 阅读设置、语言、数据保留、provider 设置集中管理。 |
| M2-07 | Todo | P0 | infra/docs/test | Docker release path | 用户能用 Docker Compose 启动；数据卷、端口、环境变量、健康检查明确。 |
| M2-08 | Todo | P0 | test | E2E MVP smoke | 自动化覆盖部署后主要 Web 流程，不依赖外网。 |
| M2-09 | Todo | P1 | frontend/backend/test | Search v0 | FTS 搜索可通过 UI 使用，支持基础筛选。 |
| M2-10 | Todo | P1 | frontend/design/test | Mobile browser polish | 手机宽度下主阅读流程可用，布局无明显溢出。 |
| M2-11 | Todo | P1 | frontend/infra/test | PWA foundation | 可安装到主屏，有基础 manifest 和图标。 |
| M2-12 | Todo | P1 | docs | User docs v0 | README 让目标用户可以部署、导入 OPML、配置 provider、备份和升级。 |
...
| ID | Status | Priority | Type | Task | Acceptance Criteria |
| --- | --- | --- | --- | --- | --- |
| M3-01 | Todo | P0 | backend/frontend/algorithm/test | Embedding provider settings | Ollama、OpenAI-compatible、自定义 HTTP provider 至少支持一种真实端到端配置；连接测试可用。 |
| M3-02 | Todo | P0 | algorithm/backend/test | Provider adapter interface | provider 统一输入输出、错误格式、维度校验、批量限制和超时策略。 |
| M3-03 | Todo | P0 | algorithm/backend/infra/test | Embedding queue and worker | 新文章进入队列；worker 批处理；失败重试；可观察待处理/失败数量。 |
| M3-04 | Todo | P0 | algorithm/backend/test | sqlite-vec index integration | embedding 写入 authority table 和 vec0 index；支持 rebuild；KNN 查询被 ranking service 使用。 |
| M3-05 | Todo | P0 | algorithm/backend/test | User profile clusters v0 | 用户行为按 1-10 档或更新后的兴趣簇模型形成画像；正负反馈和衰减有测试。 |
| M3-06 | Todo | P0 | algorithm/backend/test | Ranking v1 fusion | baseline + vector similarity + profile + penalties 融合排序；无 embedding 时 graceful fallback。 |
...
1. `R-01 / M1-09`: Web OPML import/export UI。
2. `M2-01`: Single-user auth。
3. `M2-02`: First-run setup wizard。
4. `M2-03`: Feed management UI。
5. `M2-04 / R-06`: Background refresh and job runner foundation。
6. `M3-01 / M3-02`: Embedding provider settings and adapter interface。
7. `M3-03 / M3-04`: Embedding queue and sqlite-vec production adapter。
8. `M3-05 / M3-06`: Profile Algorithm v0 and Ranking v1。
9. `M2-07 / M2-08 / M2-12`: Docker, E2E smoke, user docs。
