...
- 已完成基础工程、数据库、单用户 setup/auth/session、首次设置向导、RSS 入库、OPML API、Web OPML UI、feed folder 展示筛选、feed 管理 UI、文章列表分页、基础 Web 阅读器、文章动作、baseline ranking、推荐解释和 i18n foundation。
- 尚未完成让普通自托管用户可用的关键闭环：后台刷新、Docker 打包、设置页、embedding provider、embedding job pipeline、sqlite-vec 生产集成、Profile Algorithm v0、推荐排序 v1、E2E 测试。
...
当前状态：In Progress。单用户 auth、首次设置向导和 feed 管理闭环已完成，后台刷新、设置页、Docker、E2E 和用户文档仍待完成。
...
1. `M2-04 / R-05 / R-06`: Background refresh and job runner foundation。
2. `M2-05`: Data retention。
3. `M2-06 / R-13`: Settings page v0。
4. `M3-01 / M3-02`: Embedding provider settings and adapter interface。
5. `M3-03 / M3-04`: Embedding queue and sqlite-vec production adapter。
6. `M3-05 / M3-06`: Profile Algorithm v0 and Ranking v1。
7. `M2-07 / M2-08 / M2-12`: Docker, E2E smoke, user docs。
