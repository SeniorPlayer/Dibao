...
从 v0.1.x 升级到 v0.2.0 会新增插件相关 SQL migration，并会在启动时通过 core blocking migration gate 先完成核心迁移。保留同一个 `/data` volume 后替换镜像并重启容器即可。
...
Upgrading from v0.1.x to v0.2.0 adds plugin-related SQL migrations. Core migrations run through the blocking migration gate before the app serves normal traffic. Keep the same `/data` volume, replace the image, and restart the container.

Back up `/data/dibao.sqlite` or the whole Docker volume before upgrading. After restart, verify:
...
v0.1.x から v0.2.0 へのアップグレードでは、プラグイン関連の SQL migration が追加されます。同じ `/data` volume を使い、image tag を差し替えてコンテナを再起動してください。
